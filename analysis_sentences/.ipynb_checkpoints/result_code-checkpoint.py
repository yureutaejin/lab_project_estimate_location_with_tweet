# -*- coding: utf-8 -*-
"""
Created on Tue Aug  11 16:23:15 2020
...
with open('데이터수집.전처리코드/Data/CNNtest_SanFransisco2018.json', encoding="UTF-8") as newsfile:
...
with open('데이터수집.전처리코드/Data/boston2_user_filtered_count0.json', encoding="UTF-8") as twfile:
    twfile = twfile.read()
    ...
    try:
        ...
        for i in range(len(usertimeline)):
            ...
            for j in range(len(resultpd3)):
                if (date == resultpd3.get_value(j, "Date")):
                    count = 0
                    count_no = 0
                    keywords = []
                    keywords_no = []
                    for word in resultpd3.get_value(j, "ExtText"):
                        if is_word_in_text(word, text):
                            count += 1
                            keywords.append(word)
                    for word in resultpd3.get_value(j, "ExtText_no"):
                        if is_word_in_text(word, text):
                            count_no += 1
                            keywords_no.append(word)

                    #  n(A)+n(B)
                    unions.append(len(text.split()) + len(resultpd3.get_value(j, "ExtText")))
                    wnews.append(resultpd3.get_value(j, "Title"))
                    wtweets.append(text)
                    countlist.append(count)
                    countlist_no.append(count_no)
                    keywordlist.append(keywords)
                    keywordlist_no.append(keywords_no)
                    wkeys.append(resultpd3.get_value(j, "ExtText"))
                    wkeys_no.append(resultpd3.get_value(j, "ExtText_no"))
    except:
        continue
...
finalresult.to_excel("result_excel/keywordTop200_sort_top10rank_aftpre_sanfran2.xlsx")


# %%
def tf_vectorizer(filtered_news, filtered_tw, stop_words):
    ...
    try:
        ...
        for i in range(len(usertimeline)):
            ...
            for j in range(len(resultpd)):
                if (date == resultpd.get_value(j, "Date")):
                    newstext = resultpd.get_value(j, "Text")
                    newstext = preprocess_news(newstext)
                    tf_similarity.append(tf_vectorizer(text, newstext, stop_words))
                    tf_wnews.append(resultpd.get_value(j, "Title"))
...
tf_result.to_excel("result_excel/TF_IDF_Top200_sanfransisco.xlsx")
...
sys.path.append("/Users/jinyuntae/Desktop/Personal_project/git_download/DatabaseResearch2020")
        ...
        for j in range(len(resultpd)):
            if (date == resultpd.get_value(j, "Date")):
                try:
                    ...
                    w2v_wnews.append(resultpd.get_value(j, "Title"))
...
w2v_result.to_excel("result_excel/Word2Vec_Top200_sanfransisco.xlsx")
...
apnews_model = "/Users/jinyuntae/Desktop/Personal_project/git_download/DatabaseResearch2020/doc2vec.bin"
enwiki_model = "/Users/jinyuntae/Desktop/Personal_project/git_download/DatabaseResearch2020/doc2vec.bin"

d2v_apnews_model = Doc2Vec.load(apnews_model)
d2v_enwiki_model = Doc2Vec.load(enwiki_model)
# d2v_same_model = models.Doc2Vec.load_word2vec_format(pre_model)
...
for t in range(usernum):
    ...
    for i in range(len(usertimeline)):
        ...
        for j in range(len(resultpd)):
            if (date == resultpd.get_value(j, "Date")):
                try:
                    first_text = resultpd.get_value(j, "Text")
                    first_text = preprocess_news(first_text)
                    Title = resultpd.get_value(j, "Title")
                    second_text = text
...
d2v_ap_result.to_excel("result_excel/Doc2Vec_ap_Top200_sanfransisco.xlsx")
d2v_wiki_result.to_excel("result_excel/Doc2Vec_wiki_Top200_sanfransisco.xlsx")
